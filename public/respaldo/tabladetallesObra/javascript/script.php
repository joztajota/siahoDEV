...
function construye_combo_valueScript(){
	...
	$parte1="<select class='form-control' id ='clasificacion' name='clasificacion[]' >";
		$parte2="";
	...
	for (var a = document.querySelectorAll('table.inventory tbody tr'), i = 0; a[i]; ++i) {
		...
		if (i!=0){ 
			for (k=0;k<i;k++){
				...
				options=a[i].querySelectorAll('option');
				
				if (actual==anterior){	

				//	b = document.querySelectorAll('table.inventory tbody');
				//	fila=b[i].querySelectorAll('tr');
				//	fila2=fila[i].value;

					alert("El Oficio "+ options[anterior].innerHTML+" ya existe en su listado: "+i);
					//$("#tabla_detalle > tbody").empty();
					//$("#tabla_detalle tr:").remove();	
					tbody = document.querySelectorAll('table.inventory tbody')
					todostr = tbody[0].querySelectorAll('tr');
